...
async function start() {
    try {
        ...
        publicData = {
            ...
            isOpenned: isOpenned,
            buyPrice: buyPrice,
            lastLog: `Analisando... RSI: ${rsi.toFixed(2)}`
        };

        console.clear();
        console.log("=== BOT CRYPTO (API ONLINE) ===");
        console.log(`Preço: ${currentPrice} | RSI: ${rsi.toFixed(2)}`);
        console.log(`Posição: ${isOpenned ? "COMPRADO" : "LIQUIDO"}`);

        if (isOpenned) {
            let sellReason = null;
            if (currentPrice <= buyPrice * (1 - STOP_LOSS_PERCENT)) sellReason = "Stop Loss";
            else if (rsi > 70 && currentPrice > buyPrice) sellReason = "Take Profit (RSI > 70)";
            else if (sma13 < sma21) sellReason = "Cruzamento de Médias";
            ...
            if (sellReason) {
                ...
                publicData.lastLog = `VENDA executada: ${sellReason}`;
            }
        } else {
            if (sma13 > sma21 && rsi < 70) {
                ...
                publicData.buyPrice = currentPrice;
                publicData.lastLog = "COMPRA executada!";
            }
        }
    } catch (err) {
